// Admin only: delete a shop item (row + product photo if any).
...
module.exports = async function handler(req, res) {
  ...
  try {
    await fetch(`${url}/rest/v1/shop?id=eq.${encodeURIComponent(id)}`, { method: 'DELETE', headers });
    if (path) await fetch(`${url}/storage/v1/object/gallery/${path}`, { method: 'DELETE', headers });
